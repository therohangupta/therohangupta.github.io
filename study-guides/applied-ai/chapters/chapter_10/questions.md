---
layout: page
title: "Chapter 10 Questions: Advanced AI Techniques"
...
**How do BYOL, DINO, MoCo, and Mean Teacher fit the slow-reference pattern?**

### Sample Answer

They use a teacher or momentum encoder that changes more slowly than the online student. The student learns from targets produced by the teacher, often under different views or augmentations. The goal is representation learning, not token-level behavior imitation. Timescale separation makes the target more stable, while other design choices prevent trivial collapse.

---

## Question 28

**How can a search process act like a teacher even if it is not a neural network?**

### Sample Answer

Search can spend more compute than the deployed policy and produce improved actions, plans, or value estimates. Those outputs become training targets for a faster model. AlphaZero-style systems are the canonical example: search improves move targets, and the policy/value network learns to approximate that expensive guidance.

---

## Question 29

**What makes RMSD more data-efficient than unmasked OPSD?**

### Sample Answer

OPSD can apply token-level reverse KL wherever the teacher and student differ, including irrelevant style or phrasing differences. RMSD first selects large teacher-student disagreements and then uses a judge to keep the task-relevant positions. That concentrates gradient updates on high-signal tokens, reducing noise and improving stability.

---

## Question 30

**What could go wrong when an LLM judge selects relevance masks?**

### Sample Answer

The judge may select fluent style tokens instead of causal task tokens, miss indirect but important positions, be sensitive to prompt wording, share the same blind spots as the teacher, or behave inconsistently across slices. The mask should be evaluated directly, and the final model should be checked for target success, specificity, and broad regressions.

---

## Question 31

**Why does on-policy data matter for self-distillation?**

### Sample Answer

On-policy data comes from the student's own behavior, so the teacher gives feedback on the states and token prefixes the student actually visits. That is more targeted than training on generic demonstrations. It is like correcting a student's attempted solution rather than only showing reference answers written by someone else.

---

## Question 32

**How would you evaluate whether RMSD preserved unrelated capabilities?**

### Sample Answer

I would measure the target behavior, specificity on unrelated prompts, and broad regression suites. For a narrow behavior like `pinapple`, I would check that the model uses the new spelling only in the intended tropical-food slice, avoids it elsewhere, and does not degrade on reasoning, knowledge, safety, formatting, or product-specific evals. Teacher agreement alone is not sufficient.
